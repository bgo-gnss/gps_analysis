...
`models` · `fitting` · `velocity` · `baseline` · `preprocess` · `outliers` · `detrend` · `staged` · `deformation` · `joint` · `transient`
...
| `detrend` | ✅ implemented (2026-07-14, `docs/DESIGN_live_detrending.md` §0/§2/§5, detrend-leaf slice) | **Stored-parameter detrending — estimate once, apply anywhere**: `estimate_detrend` (**`segments=` union fit domain** — excise a transient and the coseismic step BETWEEN the flanks stays estimable, because the step filter uses the kept-epoch HULL; `max_gap_years` is per segment (on the hull the excision was the largest gap and rejected every union — the blocker), `min_span_years` is summed coverage (⊆ hull, so uniformly stricter); record gains additive `segments`/`segment_gaps` at version 1; §2.2 validity gates → `with_steps` step augmentation → `detect_outliers` BEFORE the fit (BGÓ hard rule) → final clean WLS; `detrend_method` tag `"step_augmented_robust"`/`"plain_wls"`, opaque `frame` passthrough, loud abort degrade), `DetrendEstimate.to_record` (self-contained station record: model code, step epochs, `fitted_at`/`frame`/`record_version`/`borrowed`/`refs` provenance), `trajectory_from_record`/`evaluate_record`/`apply_detrend` (pure apply at ANY epoch incl. future/borrowed-station; frame-mismatch guard; exactly invertible, raw never mutated), `select_terms` (secular/periodic views by coefficient+covariance zeroing; steps stay secular). Estimation caller (precompute/config/steps.csv/staleness) + `geo_dataread` delivery = later slices |
| `staged` | ✅ implemented (2026-08-01) | **Fit some terms here, hold others there** — the operator manoeuvre of `detrend-OLAC/detrend_test.py::katlafitlong` made declarative, and `DESIGN_live_detrending.md` §2.6 borrowing implemented rather than specified (both are ONE mechanism, differing only in the held vector's provenance). `fit_held_partition` is the atom: `y′ = y − A_h·v`, drop those columns, `_wls_solve` unchanged. **Covariance is PROPAGATED, not conditional:** `C_cond + K·C_v·Kᵀ` with `K` the WLS regression of held columns on free — Murphy–Topel is NOT needed because for two linear WLS stages on NESTED windows the cross term is *identically zero* (stage 1 fitting free+held jointly makes v̂'s error W-orthogonal to stage-2's columns). Verified 1.2e-11 vs exact propagation; the composed matrix carries −K·C_v off-blocks and is PSD, so it is a real covariance and not a zeroed stand-in. `C_v` MUST be the held block of stage 1's JOINT covariance — nothing downstream can detect otherwise. `estimate_staged` orchestrates a `Stage` plan (composition: the last stage that frees a term owns it, validated before any fitting); `stage_plan`/`stages` serialize additively at `RECORD_VERSION` 1. Measured over 200 realizations: staging wins rate AND seasonal under trend-free flank wander, and **LOSES by 25×** when the contaminant carries a net slope — that boundary is test-pinned. **Apply-only stages (2026-08-26):** a `Stage` with `free=()` + holds takes an apply path — params = the composed held values, zero covariance (nothing invented), `held_covariance="applied"` — representing the fully-borrowed station (ELDC); `fit_held_partition` keeps refusing an all-held mask (it is a fitting primitive; the path goes around it). **Guarded 2026-09-13:** a stage may only reach the apply path by freeing NOTHING — freeing a group the model has no term for now raises (per group, not on the union), because an empty `free_mask` used to select apply and emit a record indistinguishable from a deliberate borrow. Two sibling guards landed with it: a `HeldFromStage` must name a stage that actually frees or holds that group (it composed a zero labelled `"propagated"`), and `record_group_mask` now enforces `record_version` when present plus the `param_names` head against the model (it was the ONE record reader on the borrow path that enforced neither; a version-less record is still tolerated for hand-written input — see `docs/REVIEW_2026-09-13_package.md` §14). `evaluate_group_values` evaluates named secular/periodic coefficients at any epochs via the SAME `Polynomial`/`Seasonal` Term classes that build every design (**fixed 2026-09-13:** its probe was `Polynomial(degree=max(max_degree,1))`, whose names stop at `rate`, so `curvature` raised unless a `poly_m` name was seen first — a degree-2 donor, the ordinary borrow input, always raised; the probe is now the fixed degree-2 named tier and order-independent) — the basis geo_dataread's cross-station borrow re-anchoring stands on |
| `fitting` | ✅ implemented | `fit_components`, `detrend_fit`, `remove_trend`, `reject_outliers` (light exploratory clip — production path is `outliers.detect_outliers`), `with_steps` (step-augmented model factory; linear models keep the closed-form WLS path — **now NESTS**: amplitude numbering continues from the base signature instead of restarting at 1, which used to raise `duplicate parameter name`), design-width validation at the fit (a design whose column count disagrees with the model was silent until it detonated in `remove_trend`) |
| `baseline` | ✅ implemented | `slice_window`, **`slice_windows`** (union of J windows; J=1 is bit-identical to `slice_window` by delegation, which is the back-compat contract), `estimate_offset`/`remove_offset`, `estimate_step_offset` |
| `outliers` | ✅ implemented (2026-07-13, `docs/DESIGN_outlier_detection.md` §3–§4) | **Model-aware detection + signal protection**: atomics `mad_scale`/`qn_scale` (Qn = non-default O(N²) reference, Phase 1.5)/`whiten`/`standardize_robust`/`rolling_median`/`rolling_mad`/`hampel_mask`/`candidate_clusters`/`step_evidence`; `detect_outliers` orchestration — Huber step-augmented fit → global (k_g=5) + windowed-Hampel (k_w=4, time windows) identifiers → §3.4 protection (`PROTECT_FLOOR/RUN/STEP/WINDOW` + elevated-background arm) → conservative iteration with the >f_max **candidate**-fraction abort (all-False flags, loud). Always returns mask + `REASON_*`/`PROTECT_*` bitmasks + `SuspectedEvent` hints + fits; never filters, never mutates. `epoch_policy` per_component (default)/union. §8.3 release gate test-pinned (`tests/test_outliers.py`). **Step-protection repair (2026-07-27, §11):** the NaN arm of the step rule is a POLICY (`protect_on_indeterminate`, default True), not an unreachable branch of `step_evidence_sigma`; a thin flank falls back to the nearest 3 usable samples within `step_flank_max_reach_days` (default 60 d) **only across a genuine data gap** — a candidate-saturated flank (fast transient) stays NaN and keeps protecting; opt-in `step_magnitude_ratio` releases a step protection the excursion dwarfs. Measured +10.8 % flagged over 20 stations. **Scale handling (2026-08-01):** `freeze_scale` (default True) holds the robust scale at its FIRST-sweep estimate — re-estimating each sweep is the classic swamping feedback (trimmed scatter → tighter threshold → flags epochs that were never anomalous); `scale_floor_fraction` (default 0.02, relative to the MEDIAN LOCAL scale) guards the Pearson 2016 §3 MAD-implosion. The reference matters: against the GLOBAL scale the smallest legitimate local scale spans 0.0020–0.166 across the working set, because unrest inflates ŝ and quiet stretches look pathological — a global-referenced 0.05 suppressed 36 % of SENG's East epochs whose windows held entirely distinct values |
...
---
*Last reviewed: 2026-09-13 (package-wide parallel code review — `docs/REVIEW_2026-09-13_package.md`:
~42 findings across all 16 modules, the load-bearing ones reproduced with numbers. The four branch
blockers in `staged` are FIXED here (apply-path guard, HeldFromStage ownership, record_group_mask
contract, evaluate_group_values curvature; 11 new tests, 9 fail against the pre-fix source).
Outstanding and prioritized in that document: the borrowed-secular datum leak (leaf + gps_api
precompute), near-vertical Okada kernels, VARPRO amplitude covariance, `freeze_scale` not freezing
the global identifier, the BPD2 inflate→deflate chain freeze, and the median-of-30 transient
baseline. Verified clean and not worth re-checking: the LOS sign convention, the analytic joint
Jacobian, χ²/dof consistency, classifier totality, the leaf guard, and `preprocess` parity.)
Previous 2026-08-26 (staged: apply-only stages — `Stage(free=())` +
holds applies the composed held values without fitting, `held_covariance
= "applied"`, covariance all-zero (no invented uncertainty); the all-borrowed
station of design §2.6 was unrepresentable before. New `evaluate_group_values`:
name-driven secular/periodic evaluation on the Term classes, powering
geo_dataread's cross-station borrow re-anchoring. Previous 2026-08-01: NEW `staged` module — held-partition fitting with the
PROPAGATED two-stage covariance `C_cond + K·C_v·Kᵀ` (cross term identically zero on
nested windows, so Murphy–Topel is unnecessary; verified 1.2e-11) and `estimate_staged`,
which turns katlafitlong into a two-line plan and implements §2.6 borrowing as the same
mechanism. Also: `estimate_detrend(segments=)` union fit domains with per-segment
`max_gap_years` and summed-coverage `min_span_years`; `baseline.slice_windows`;
`with_steps` now nests; design-width validation; `velocity` resolves DERIVED designs
(it silently dropped step-augmented models to curve_fit); `RECORD_VERSION` split into
emit vs `SUPPORTED_RECORD_VERSIONS`; `outliers` freezes the robust scale across sweeps
and floors the local scale against MAD implosion. Established-algorithm audit vs HECTOR
(GPL-3, source read): ours is measurably better for curation — global IQR on OLS
residuals vs robust Huber + windowed Hampel + signal protection; isolated-spike TP at
6/8/10 mm 0.11/0.42/0.73 vs 0.25/0.56/0.79 — and Bos et al. 2013 is already cited
throughout `noise`. HECTOR's `findoffset` BIC_c is queued as a later phase; DOGEx
(Gazeaux 2013) is the benchmark any detector gets scored on. Previous 2026-07-14: stored-parameter detrending leaf per
